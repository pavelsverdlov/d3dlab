...
namespace D3DLab.Std.Engine.Core.Utilities.Helix {
    ...
    public class MeshBuilder {
        ...
        /// </summary>
        private static readonly Dictionary<int, AbstractGeometry3D> UnitSphereCache = new Dictionary<int, AbstractGeometry3D>();
        ...
        public List<Vector3> Normals { get { return this.normals; } set { this.normals = value; } }

        public List<Vector3> Tangents { get { return this.tangents; } set { this.tangents = value; } }

        public List<Vector3> BiTangents { get { return this.bitangents; } set { this.bitangents = value; } }

        public List<Vector2> TextureCoordinates { get { return this.textureCoordinates; } set { this.textureCoordinates = value; } }

        public List<int> TriangleIndices { get { return this.triangleIndices; } }
        ...
        public void ComputeNormalsAndTangents(MeshFaces meshFaces, bool tangents = false) {
            if (!this.HasNormals & this.positions != null & this.triangleIndices != null) {
                this.normals = this.positions.CalculateNormals(this.triangleIndices);
            ...
            switch (meshFaces) {
                case MeshFaces.Default:
                    if (tangents & this.HasNormals & this.textureCoordinates != null) {
                        ComputeTangents(this.positions, this.normals, this.textureCoordinates, this.triangleIndices, out var t1, out var t2);
                        this.tangents = t1;
                        this.bitangents = t2;
                    }
                    break;
                case MeshFaces.QuadPatches:
                    if (tangents & this.HasNormals & this.textureCoordinates != null) {
                        ComputeTangentsQuads(this.positions, this.normals, this.textureCoordinates, this.triangleIndices, out var t1, out var t2);
                        this.tangents = t1;
                        this.bitangents = t2;
                    }
                    break;
        ...
        private static void ComputeNormals(List<Vector3> positions, List<int> triangleIndices, out List<Vector3> normals) {
            ...
            for (int t = 0; t < triangleIndices.Count; t += 3) {
                var i1 = triangleIndices[(int)t];
                var i2 = triangleIndices[(int)t + 1];
                var i3 = triangleIndices[(int)t + 2];

                var v1 = positions[(int)i1];
                var v2 = positions[(int)i2];
                var v3 = positions[(int)i3];
        ...
        public static void ComputeTangents(IList<Vector3> positions, IList<Vector3> normals, IList<Vector2> textureCoordinates, IList<int> triangleIndices,
            ...
            for (int t = 0; t < triangleIndices.Count; t += 3) {
                ...
                var w1 = textureCoordinates[(int)i1];
                var w2 = textureCoordinates[(int)i2];
                var w3 = textureCoordinates[(int)i3];
            ...
            var atangents = new Vector3[positions.Count];
            var abitangents = new Vector3[positions.Count];
            for (int i = 0; i < positions.Count; i++) {
                var n = normals[i];
                var t = tan1[i];
            ...
            tangents = new List<Vector3>(atangents);
            bitangents = new List<Vector3>(abitangents);
        }
        ...
        public static void ComputeTangentsQuads(IList<Vector3> positions, IList<Vector3> normals, IList<Vector2> textureCoordinates, IList<int> indices,
            ...
            for (int t = 0; t < indices.Count; t += 4) {
                ...
                var v2 = positions[(int)i2];
                var v3 = positions[(int)i3];
                var v4 = positions[(int)i4];

                var w1 = textureCoordinates[(int)i1];
                var w2 = textureCoordinates[(int)i2];
                var w3 = textureCoordinates[(int)i3];
                var w4 = textureCoordinates[(int)i4];
        ...
        public static void ComputeTangents(AbstractGeometry3D meshGeometry) {
            ComputeTangents(meshGeometry.Positions, meshGeometry.Normals, meshGeometry.TextureCoordinates, meshGeometry.Indices.Select(x=>(int)x).ToList(), out List<Vector3> t1, out List<Vector3> t2);
        ...
        /// </summary>
        public AbstractGeometry3D ToGeometry3D() {
            if (this.HasTangents && this.tangents.Count == 0) {
                ComputeTangents(this.positions, this.normals, this.textureCoordinates, this.triangleIndices, out List<Vector3> tan, out List<Vector3> bitan);
        ...
        public static IList<Vector2> GetCircle(int thetaDiv) {
            if (!CircleCache.TryGetValue(thetaDiv, out IList<Vector2> circle)) {
        ...
        public void AddArrow(Vector3 point1, Vector3 point2, double diameter, double headLength = 3, int thetaDiv = 18, float headWidth = 2) {
            ...
            var pc = new List<Vector2>
                {
                    ...
                    new Vector2(length - (float)(diameter * headLength), r * headWidth),
        ...
        public void AddDashedArrow(Vector3 point1, Vector3 point2, double diameter, double dashLenght, double skipLenght, double headLength = 3, int thetaDiv = 18, float headWidth = .5f) {
            var dir = point2 - point1;
            var length = Vector3.Distance(point1, point2);
            var r = (float)diameter / 2;
            var lineLenght = length - (float)headLength;

            var pc = new List<Vector2>();

            var segmentLenght = dashLenght + skipLenght;
            for (float p = 0; p < lineLenght; p += (float)segmentLenght) {
                pc.Add(new Vector2(p, 0));
                pc.Add(new Vector2(p, r));
                var stop = (p + dashLenght < lineLenght) ? p + (float)dashLenght : lineLenght;
                pc.Add(new Vector2(stop, r));
                pc.Add(new Vector2(stop, 0));
            }

            pc.Add(new Vector2(lineLenght, 0));
            pc.Add(new Vector2(lineLenght, headWidth));
        ...
        /// </param>
        public void AddBoundingBox(Veldrid.Utilities.BoundingBox boundingBox, double diameter) {
            var corners = boundingBox.GetCorners().ToArray();

            var p0 = corners[0]; //new Vector3((float)boundingBox.X, (float)boundingBox.Y, (float)boundingBox.Z);
            var p1 = corners[1]; //new Vector3((float)boundingBox.X, (float)boundingBox.Y + (float)boundingBox.SizeY, (float)boundingBox.Z);
            var p2 = corners[2]; //new Vector3((float)boundingBox.X + (float)boundingBox.SizeX, (float)boundingBox.Y + (float)boundingBox.SizeY, (float)boundingBox.Z);
            var p3 = corners[3]; //new Vector3((float)boundingBox.X + (float)boundingBox.SizeX, (float)boundingBox.Y, (float)boundingBox.Z);
            var p4 = corners[4]; //new Vector3((float)boundingBox.X, (float)boundingBox.Y, (float)boundingBox.Z + (float)boundingBox.SizeZ);
            var p5 = corners[5]; //new Vector3((float)boundingBox.X, (float)boundingBox.Y + (float)boundingBox.SizeY, (float)boundingBox.Z + (float)boundingBox.SizeZ);
            var p6 = corners[6]; //new Vector3((float)boundingBox.X + (float)boundingBox.SizeX, (float)boundingBox.Y + (float)boundingBox.SizeY, (float)boundingBox.Z + (float)boundingBox.SizeZ);
            var p7 = corners[7]; //new Vector3((float)boundingBox.X + (float)boundingBox.SizeX, (float)boundingBox.Y, (float)boundingBox.Z + (float)boundingBox.SizeZ);
        ...
        /// The 3-D "rectangle".
        /// </param>
        public void AddBox(Veldrid.Utilities.BoundingBox rectangle, BoxFaces faces = BoxFaces.All) {
        ...
        public void AddFacePZ() {
            ...
            var i0 =(int) this.positions.Count;
            var indices = new int[]
            {
                i0+0,i0+3,i0+2,
                i0+0,i0+2,i0+1,
        ...
        public void AddFaceNZ() {
            ...
            int i0 = (int)this.positions.Count;
            var indices = new int[]
            {
                i0+0,i0+3,i0+2,
                i0+0,i0+2,i0+1,
        ...
        /// </param>
		/// <param name="depth">
		/// The distance from the face to the opposite face of the cube.
        /// </param>
        /// <param name="width">
        ...
        /// </param>
        private void AddCubeFace(Vector3 center, Vector3 normal, Vector3 up, double depth, double width, double height) {
        ...
        public void AddExtrudedGeometry(IList<Vector2> points, Vector3 xaxis, Vector3 p0, Vector3 p1) {
            ...
            int index0 = (int)this.positions.Count;
            int np = 2 * (int)points.Count;
        ...
        public void AddLoftedGeometry(IList<IList<Vector3>> positionsList, IList<IList<Vector3>> normalList, IList<IList<Vector2>> textureCoordinateList) {
            ...
            for (int i = 0; i + 1 < positionsList.Count; i++) {
                for (int j = 0; j + 1 < n; j++) {
                    ...
                    var i2 = i1 + 1;
                    var i3 = i0 + 1;
                    this.triangleIndices.Add((int)i0);
                    this.triangleIndices.Add((int)i1);
                    this.triangleIndices.Add((int)i2);

                    this.triangleIndices.Add((int)i2);
                    this.triangleIndices.Add((int)i3);
                    this.triangleIndices.Add((int)i0);
                }
            }
        }
        ...
        /// The points of the polygon.
        /// </param>
        /// <param name="pointize"></param>
        /// <remarks>
        /// 
        /// </remarks>
        public void AddPoints(IList<Vector3> points, double pointize) {
            foreach (var vector3 in points) {
                AddBox(vector3, pointize, pointize, pointize, BoxFaces.All);
            }
        }
        ...
        public void AddTriangle(Vector3 p0, Vector3 p1, Vector3 p2, Vector2 uv0, Vector2 uv1, Vector2 uv2) {
            ...
            this.triangleIndices.Add((int)i0 + 0);
            this.triangleIndices.Add((int)i0 + 1);
            this.triangleIndices.Add((int)i0 + 2);
        }
        ...
        public void AddTriangleFan(IList<Vector3> fanPositions, IList<Vector3> fanNormals = null, IList<Vector2> fanTextureCoordinates = null) {
            ...
            var index0 = this.positions.Count;
            this.positions.AddRange(fanPositions);

            if (this.textureCoordinates != null && fanTextureCoordinates != null) {
                this.textureCoordinates.AddRange(fanTextureCoordinates);
            }

            if (this.normals != null && fanNormals != null) {
                this.normals.AddRange(fanNormals);
            }
        ...
        /// </param>
        public void AddTube(IList<Vector3> path, IList<double> values, IList<double> diameters, IList<Vector2> section, bool isTubeClosed, bool isSectionClosed) {
            ...
            for (int i = 0; i < pathLength; i++) {
                ...
                while (forward.LengthSquared() < 1e-3 && ++i1 < pathLength) {
                ...
                for (int j = 0; j < sectionLength; j++) {
                    ...
                    if (this.textureCoordinates != null) {
                        this.textureCoordinates.Add(values != null ? new Vector2((float)values[i % valuesCount], (float)(j / (sectionLength - 1))) : new Vector2());
                    }
                }
            }
        ...
        public void Append(Components.GeometryComponent geo) {
            this.Append(geo.Positions, geo.Indices, geo.Normals, geo.TextureCoordinates,colorsToAppend: geo.Colors);
        }
        ...
        /// The mesh.
        /// </param>
        public void Append(AbstractGeometry3D mesh) {
            if (mesh == null) {
                throw new ArgumentNullException("mesh");
            }

            this.Append(mesh.Positions, mesh.Indices.Select(x=>(int)x).ToList(), mesh.Normals, mesh.TextureCoordinates, mesh.Colors);
        }

        private void Append(List<Vector3> positionsToAppend, List<int> triangleIndicesToAppend, List<Vector3> normalsToAppend, List<Vector2> textureCoordinatesToAppend, List<Vector4> colorsToAppend) {
            ...
            if (colorsToAppend != null && colors != null) {
                this.colors.AddRange(colorsToAppend);
            }
            ...
            if (this.textureCoordinates != null && textureCoordinatesToAppend != null) {
                textureCoordinates.AddRange(textureCoordinatesToAppend);
            }

            this.triangleIndices.Capacity = this.triangleIndices.Count + triangleIndicesToAppend.Count;
            for (int i = 0; i < triangleIndicesToAppend.Count; i++) {
                this.triangleIndices.Add(index0 + triangleIndicesToAppend[i]);
            }
        }
        ...
        public void Append(
            ...
            IList<Vector4> colorsToAppend = null) {
            if (positionsToAppend == null) {
                throw new ArgumentNullException("positionsToAppend");
            }
            ...
            if (colors != null && colorsToAppend != null) {
                this.colors.AddRange(colorsToAppend);
            }
        ...
        /*public void ChamferCorner(Vector3 p, double d, double eps = 1e-6, IList<Vector3> chamferPoints = null) {
            ...
            for (int i = 0; i < ntri; i += 3) {
                ...
                if (!plane.Intersects(ref line1, out Vector3 p01)) {
                    continue;
                }

                if (!plane.Intersects(ref line2, out Vector3 p02)) {
                    continue;
                }
        ...
        /// A unit sphere mesh.
        /// </returns>
        private static AbstractGeometry3D GetUnitSphere(int subdivisions) {
        ...
        private Vector3 FindCornerNormal(Vector3 p, double eps) {
            ...
            for (int i = 0; i < this.triangleIndices.Count; i += 3) {
                ...
                int i1 = i + 1;
                int i2 = i + 2;
                var p0 = this.positions[(int)this.triangleIndices[i0]];
                var p1 = this.positions[(int)this.triangleIndices[i1]];
                var p2 = this.positions[(int)this.triangleIndices[i2]];
        ...
        private void NoSharedVertices() {
            ...
            for (int i = 0; i < this.triangleIndices.Count; i += 3) {
                ...
                int i1 = i + 1;
                int i2 = i + 2;
                int index0 = this.triangleIndices[(int)i0];
                int index1 = this.triangleIndices[(int)i1];
                int index2 = this.triangleIndices[(int)i2];
                var p0 = this.positions[(int)index0];
                var p1 = this.positions[(int)index1];
                var p2 = this.positions[(int)index2];
                p.Add(p0);
                p.Add(p1);
                p.Add(p2);
                ti.Add(i0);
                ...
                ti.Add(i2);
                if (n != null) {
                    n.Add(this.normals[(int)index0]);
                    n.Add(this.normals[(int)index1]);
                    n.Add(this.normals[(int)index2]);
                }

                if (tc != null) {
                    tc.Add(this.textureCoordinates[(int)index0]);
                    tc.Add(this.textureCoordinates[(int)index1]);
                    tc.Add(this.textureCoordinates[(int)index2]);
                }
            }
        ...
        private void Subdivide4() {
            ...
            int ip = (int)this.positions.Count;
            int ntri = (int)this.triangleIndices.Count;
            for (int i = 0; i < ntri; i += 3) {
                int i0 = this.triangleIndices[(int)i];
                int i1 = this.triangleIndices[(int)i + 1];
                int i2 = this.triangleIndices[(int)i + 2];
                var p0 = this.positions[(int)i0];
                var p1 = this.positions[(int)i1];
                var p2 = this.positions[(int)i2];
                var v01 = p1 - p0;
                var v12 = p2 - p1;
                var v20 = p0 - p2;
                var p01 = p0 + (v01 * 0.5f);
                ...
                if (this.textureCoordinates != null) {
                    var uv0 = this.textureCoordinates[(int)i0];
                    var uv1 = this.textureCoordinates[(int)i0 + 1];
                    var uv2 = this.textureCoordinates[(int)i0 + 2];
                    var t01 = uv1 - uv0;
                    var t12 = uv2 - uv1;
                    var t20 = uv0 - uv2;
                ...
                this.triangleIndices[(int)i + 1] = i01;
                this.triangleIndices[(int)i + 2] = i20;
        ...
        private void SubdivideBarycentric() {
            ...
            int im = (int)this.positions.Count;
            int ntri = (int)this.triangleIndices.Count;
            for (int i = 0; i < ntri; i += 3) {
                int i0 = this.triangleIndices[(int)i];
                int i1 = this.triangleIndices[(int)i + 1];
                ...
                // TriangleIndices[i ] = i0;
                this.triangleIndices[(int)i + 1] = i01;
                this.triangleIndices[(int)i + 2] = im;
        ...
        public void AddTorus(float r1, float r2, int d1 = 36, int d2 = 16, Vector3 center = default(Vector3), Vector3 normal = default(Vector3)) {
            ...
            float da2 = (float)(Math.PI * 2 / d2);

            var n1 = normal.FindAnyPerpendicular();
            n1.Normalize();

            var circlePoints = new Vector3[d2];
            var circleNormals = new Vector3[d2];
            var circleAxis = Vector3.Cross(normal, n1);
            var m2 = circleAxis.RotationAround(da2, center + n1 * r1);
            Vector3 p = center + n1 * (r1 - r2);
            Vector3 pn = -n1;
            for (int i = 0; i < d2; i++) {
                circlePoints[i] = p;
                circleNormals[i] = pn;
                p = Vector3.Transform(p, m2);
                pn = Vector3.TransformNormal(pn, m2);
            }

            float da1 = (float)(Math.PI * 2 / d1);
            var m1 = normal.RotationAround(da1, center);
            var startIdx = this.positions.Count;
            for (int i = 0; i < d1; i++) {
                this.positions.AddRange(circlePoints);
                circlePoints = circlePoints.Transform(ref m1);
                if (this.normals != null) {
                    this.normals.AddRange(circleNormals);
                    for (int j = 0; j < circleNormals.Length; j++) {
                        circleNormals[j] = Vector3.TransformNormal(circleNormals[j], m1);
                    }
                }

                int idx1 = (int)(positions.Count - circlePoints.Length);
                int idx2 = i == d1 - 1 ? (int)startIdx : (int)positions.Count;
                for (int j = 0; j < circlePoints.Length; j++) {
                    var last = j == circlePoints.Length - 1;
                    int i1 = idx1 + j;
                    int i2 = last ? idx1 : idx1 + j + 1;
                    int i3 = idx2 + j;
                    int i4 = last ? idx2 : idx2 + j + 1;

                    this.triangleIndices.Add(i1);
                    this.triangleIndices.Add(i2);
                    this.triangleIndices.Add(i4);
                    this.triangleIndices.Add(i1);
                    this.triangleIndices.Add(i4);
                    this.triangleIndices.Add(i3);
                }
            ...
            //    textureCoordinates.SetSize(this.positions.Count);
            //}
        }

        public void AddDashedTorus(float radius, float tubeRadius, float dashLength, float skipLenght, Vector3 center = default(Vector3), Vector3 normal = default(Vector3)) {
            var occlusionAxis = new Vector3(0, 0, 1);
            var circleLength = 2 * Math.PI * radius;
            var dashAngle = (Math.PI * 2) / (circleLength / dashLength);
            ...
            for (double i = 0; i < Math.PI * 2; i += segmentAngle) {
                var pointStart = new Vector3(
                    center.X + (float)(radius * Math.Cos(i)),
                    center.Y + (float)(radius * Math.Sin(i)),
                    0);

                var pointStop = new Vector3(
                    center.X + (float)(radius * Math.Cos((i + dashAngle))),
                    center.Y + (float)(radius * Math.Sin(i + dashAngle)),
                    0);


                pointStart = Vector3.Transform(pointStart, matrix);
                pointStop = Vector3.Transform(pointStop, matrix);

                AddCylinder(pointStart, pointStop, tubeRadius);
            }
        }
