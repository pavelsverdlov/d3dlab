...
namespace D3DLab.Std.Engine {
    ...
    public class TexturedGeometryGraphicsComponent : D3DComponent, IRenderableComponent {
        ...
        public TexturedGeometryGraphicsComponent(ShaderInfo[] shaders, Geometry3D geometry, TextureInfo texture) {
    ...
    public class Game {
        ...
        public void Run(IEntityRenderNotify notify) {
            ...
            while (window.IsActive) {
                var deltaSeconds = speed.Elapsed.TotalSeconds;
                speed.Restart();
                ...
                try {
                    ...
                    var snapshot = new SceneSnapshot(Context, ishapshot, TimeSpan.FromSeconds(deltaSeconds));
                ...
                var perfomance = Context.GetEntityManager().GetEntities()
                    .Single(x => x.GetComponent<EngineInfoBuilder.PerfomanceComponent>() != null)
                    .GetComponent<EngineInfoBuilder.PerfomanceComponent>();

                previousFrameTicks = sw.ElapsedTicks;
                //sw.Stop();

                perfomance.ElapsedMilliseconds = deltaSeconds;
