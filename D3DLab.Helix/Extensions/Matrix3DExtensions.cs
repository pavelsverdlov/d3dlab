...
namespace HelixToolkit.Wpf.SharpDX
{
	public static class Matrix3DExtensions
	{
		...
		public static Matrix3D CheckForNaN(this Matrix3D m)
		{
			if (m.HasNaN())
			{
				Debug.WriteLine("= Matrix.HasNaN =");
			}
			return m;
		}

		public static bool HasNaN(this Matrix3D m)
		{
			if (m.M11 == double.NaN || m.M12 == double.NaN || m.M13 == double.NaN || m.M14 == double.NaN
				|| m.M21 == double.NaN || m.M22 == double.NaN || m.M23 == double.NaN || m.M24 == double.NaN
				|| m.M31 == double.NaN || m.M32 == double.NaN || m.M33 == double.NaN || m.M34 == double.NaN
				|| m.M44 == double.NaN
				|| m.OffsetX == double.NaN || m.OffsetY == double.NaN || m.OffsetZ == double.NaN)
			{
				return true;
			}
			return false;
        ...
        private static bool IsZero(this double v)
        {
            return Math.Abs(v) < 0.00001;
        }

        public static bool IsZeroMatrix(this Matrix3D m)
        {
            if (m.M11.IsZero() || m.M12.IsZero() || m.M13.IsZero() || m.M14.IsZero()
                || m.M21.IsZero() || m.M22.IsZero() || m.M23.IsZero() || m.M24.IsZero()
                || m.M31.IsZero() || m.M32.IsZero() || m.M33.IsZero() || m.M34.IsZero()
                || m.M44.IsZero()
                || m.OffsetX.IsZero() || m.OffsetY.IsZero() || m.OffsetZ.IsZero())
            {
                return true;
            }
            return false;
		...
		public static Transform3D ToTransform(this Matrix3D matrix)
		{
			return new MatrixTransform3D(matrix);
		}

        public static Transform3D ToTransform(this Matrix matrix)
        {
