...
namespace HelixToolkit.Wpf.SharpDX.Controllers {
    public abstract class CameraViewControllerHandler<T> : ICameraViewControllerHandler where T : Camera {
        ...
        public Point3D RotateCenter { get { return Controller.RotateCenter; } }
        ...
        public Vector3 Look {
            get {
                var value = Camera.LookDirection.ToVector3();
                value.Normalize();
                return value;
            }
            set {
                if (value.IsNaN())
                    return;
                value.Normalize();
                Camera.LookDirection = value.ToVector3D();
            }
        }

        public Vector3 Up {
            get {
                var value = Camera.UpDirection.ToVector3();
                value.Normalize();
                return value;
            }
            set {
                if (value.IsNaN())
                    return;
                value.Normalize();
                Camera.UpDirection = value.ToVector3D();
            }
        }

        public Vector3 Position {
            get {
                var value = Camera.Position.ToVector3();
                return value;
            }
            set {
                if (value.IsNaN())
                    return;
                Camera.Position = value.ToPoint3D();
            }
        }
        ...
        public virtual void Rotate(Vector3 axis, float angle) {
            ...
            var matrixRotate = axis.RotateAroundAxis(angle, RotateCenter.ToVector3());
            if (matrixRotate.IsIdentity)
                return;
        ...
        private Matrix GetMatrixRotate3D(float dx, float dy) {
            ...
            var orthCamera = Camera as OrthographicCamera;
        ...
        private Matrix ToRotateMatrix(Vector3 axis, float angle) {
            var matrixRotate = SDXCommonExtensions.RotateAroundAxis(axis, angle, RotateCenter.ToVector3());
            return matrixRotate;
        }
