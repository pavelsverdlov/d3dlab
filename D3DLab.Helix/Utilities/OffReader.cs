    ...
    public class OffReader : IModelReader
    {
        ...
        private void Load(Stream s)
        {
            using (var reader = new StreamReader(s))
            {
                ...
                var dblValues = new double[vertexDimension];
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    if (nextLineContainsVertexDimension)
                    {
                        ...
                        if (vertexDimension > dblValues.Length)
                            dblValues = new double[vertexDimension];
                        nextLineContainsVertexDimension = false;
                        continue;
                    }

                    //if (line.Contains("OFF"))
                    if (line.Length >= 3 && line[0] == 'O' && line[1] == 'F' && line[1] == 'F')
                    {
                        // Tlk: not use
                    ...
                    if (this.Vertices.Count < numberOfVertices)
                    {
                        ...
                        this.Vertices.Add(new Point3D((float)dblValues[0], (float)dblValues[1], (float)dblValues[2]));
        ...
        private static int[] GetIntValues(string input)
        {
            ...
            {
                result[i] = int.Parse(fields[i], CultureInfo.InvariantCulture);
            }
