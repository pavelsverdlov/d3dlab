...
namespace HelixToolkit.Wpf.SharpDX
{
    ...
    public class ObjExporter : Exporter
    {
        ...
        public ObjExporter(FileInfo outputFileName, FileInfo textureImagePath) {
            this.SwitchYZ = true;
            this.ExportNormals = false;

            var mtlPath = Path.ChangeExtension(outputFileName.FullName, ".mtl");
            string mtlFilename = Path.GetFileName(mtlPath);
            this.directory = outputFileName.DirectoryName;

            this.writer = new StreamWriter(outputFileName.FullName);
            this.mwriter = new StreamWriter(mtlPath);

            if (textureImagePath != null) {
                this.mwriter.WriteLine("newmtl Textured");
                this.mwriter.WriteLine("map_Ka " + textureImagePath.FullName);
                this.mwriter.Flush();
            }

            this.writer.WriteLine("mtllib ./" + mtlFilename);
        }
          public ObjExporter(FileInfo textureImagePath, StreamWriter output, StreamWriter mtl) {
            this.SwitchYZ = true;
            this.ExportNormals = false;
            
            this.writer = output;
            this.mwriter = mtl;
              var name = textureImagePath.Name;
            if(textureImagePath != null) {
                this.mwriter.WriteLine("newmtl Textured");
                this.mwriter.WriteLine("map_Ka " + name);
                this.mwriter.Flush();
            }
            var mtlPath = Path.ChangeExtension(textureImagePath.Name, ".mtl");
            string mtlFilename = Path.GetFileName(mtlPath);

            this.writer.WriteLine("usemtl Textured");
            this.writer.WriteLine("mtllib " + mtlFilename);
