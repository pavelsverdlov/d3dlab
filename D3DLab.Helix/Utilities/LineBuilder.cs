...
namespace HelixToolkit.Wpf.SharpDX
{
	public class LineBuilder
	{
		...
		/// <summary>
		/// Create a builder and add a point list to it
		/// </summary>
		/// <param name="points"></param>
		/// <param name="closed"></param>
		public LineBuilder(List<Vector3> points, bool closed = false)
			: this()
		{
			Add(points, closed);
		}
		...
		/// <summary>
		/// Add a point list to a builder
		/// </summary>
		/// <param name="points"></param>
		/// <param name="closed"></param>
		public void Add(IEnumerable<Vector3> points, bool closed = false)
		{
		...
		{
			var bb = mesh.Positions.ToBoundingBox();
		...
		public static float GetPointToLineDistance2D(ref Vector3 pt, ref Vector3 p0, ref Vector3 p1, out Vector3 closest, out float t)
		{
			...
			else
			{
				...
				dx = pt.X - /*closest.X*/(p0.X + t * dx);
				dy = pt.Y - /*closest.Y*/(p0.Y + t * dy);
			}

			return (float)Math.Sqrt(dx * dx + dy * dy);
		}

		public static float GetPointToLineDistance2D(ref Vector3 pt, ref Vector3 p0, ref Vector3 p1, out float t)
		{
			float dx = p1.X - p0.X;
			float dy = p1.Y - p0.Y;
			if (Math.Abs(dx) < float.Epsilon && Math.Abs(dy) < float.Epsilon)
