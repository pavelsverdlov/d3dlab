...
namespace HelixToolkit.Wpf.SharpDX
{
	...
	public static class CameraExtensions
	{
		...
		public static void AnimateTo(
			...
			double animationTime, bool useAcceleration = true)
		{
			...
			Action<AnimationTimeline, DependencyProperty> startAnimation = (anim, prop) =>
			{
				if (useAcceleration)
				{
				...
				anim.Completed += (s, a) => { camera.BeginAnimation(prop, null); };
				camera.BeginAnimation(prop, anim, HandoffBehavior.Compose);
			};

			//Tlk: normalize up vector.
			var look = newDirection;
			var up = newUpDirection;
			look.Normalize();
			up.Normalize();
			var v = Vector3D.CrossProduct(look, up);
			v.Normalize();
			newUpDirection = Vector3D.CrossProduct(v, newDirection);
			...
			if (animationTime > 0)
			{
				...
				if (fromPosition != newPosition)
					startAnimation(new Point3DAnimation(fromPosition, newPosition, duration), ProjectionCamera.PositionProperty);

				if (fromDirection != newDirection)
					startAnimation(new Vector3DAnimation(fromDirection, newDirection, duration), ProjectionCamera.LookDirectionProperty);

				if (fromUpDirection != newUpDirection)
					startAnimation(new Vector3DAnimation(fromUpDirection, newUpDirection, duration), ProjectionCamera.UpDirectionProperty);
			}
			else
			{
		...
		public static void AnimateWidth(this OrthographicCamera camera, double newWidth, double animationTime)
		{
			...
			if (animationTime > 0)
			{
				...
				a1.Completed += (s, a) => { camera.BeginAnimation(OrthographicCamera.WidthProperty, null); };
