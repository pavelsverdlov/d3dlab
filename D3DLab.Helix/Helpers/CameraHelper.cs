...
namespace HelixToolkit.Wpf.SharpDX.Helpers
{
    ...
    public static class CameraHelper
    {
        ...
        /// The animation time.
        /// </param>
        public static void LookAt(this SharpDX.OrthographicCamera camera, Point3D target, Vector3D newLookDirection, Vector3D newUpDirection, double animationTime) {
            Point3D newPosition = target - newLookDirection;
            if(camera is SharpDX.OrthographicCamera) {
                AnimateTo(camera as SharpDX.OrthographicCamera, newPosition, newLookDirection, newUpDirection, animationTime);
            }
        }

        private static void AnimateTo(SharpDX.OrthographicCamera camera, Point3D newPosition, Vector3D newDirection, Vector3D newUpDirection, double animationTime){
            var fromPosition = camera.Position;
            var fromDirection = camera.LookDirection;
            var fromUpDirection = camera.UpDirection;
