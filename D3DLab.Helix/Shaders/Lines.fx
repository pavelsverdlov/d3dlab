...
// GLOBAL FUNCTIONS
//--------------------------------------------------------------------------------------
int CalcPlanesLines(PSInputLS input)
{
	int result = -1;
	for (int i = 0; i < nPlaneCount; i++)
	{
		float4 normal = vPlaneNormals[i];
		float d = vPlaneDs[i];
		float res = normal.x * input.wp.x + normal.y * input.wp.y + normal.z * input.wp.z + d;
		if (res < 0)
		{
			discard;
		}
...
//PSInputLS VShaderLinesToPS(VSInputLS input)
//{
...
//[maxvertexcount(6)]
//void GShaderLinesEx(line GSInputLS input[2], inout TriangleStream<PSInputLS> outStream)
//{
//	PSInputLS output = (PSInputLS)0;
...
{
	float4 I;
	int pRes = CalcPlanesLines(input);
	if (pRes >= 0)
	{
		I = vPlaneColors[pRes];
		//I = float4(1, 0, 0, 1);
...
technique11 RenderLines
{
	pass P0
	{
		...
		//SetVertexShader(CompileShader(vs_4_0, VShaderLinesToPS()));
		SetHullShader(NULL);
